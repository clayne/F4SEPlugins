...
public:
	...
	UInt64	unkVRB70[(0xc48 - 0x7D8) / 8];		// B70

	tArray<ObjectiveData> objData;				// C48

	UInt64	unkVRC60[(0xFE0 - 0xC60) / 8];		// C60

	ActorEquipData	* playerEquipData;			// FE0 - First person?
	NiNode			* firstPersonSkeleton;		// FE8
	UInt64	unkB68[(0xD00-0xB80)/8];			// FF0
	tArray<BGSCharacterTint::Entry*> * tints;	// 
	UInt64	unkC90[(0xE10-0xCF8)/8];			// 
};

extern RelocPtr <PlayerCharacter*> g_player;

STATIC_ASSERT(offsetof(PlayerCharacter, menuOpenClose) == 0x490);
STATIC_ASSERT(offsetof(PlayerCharacter, objData) == 0xc48);
